...
@njit(debug=False)
def decode_continuous_with_vote_diff(y_thermometer, scores_batch, n, num_bands=5):
    """
    Combines thermometer decoding with score-based interpolation.
    It uses the difference in vote sums (scores) to determine the precise 
    floating point position within the discrete band.

    Args:
        y_thermometer (np.ndarray): Shape [B, 2*n*num_bands]
        scores_batch (np.ndarray): Shape [B, 1 + 2*2*n]
                                   [Baseline, Feat0_Up, Feat0_Down, ...]
        n (int): Feature count parameter
        num_bands (int): Number of bands used for encoding

    Returns:
        np.ndarray: Shape [B, 2*n]. Continuous float values in range [-3, 3]
                    adjusted based on the vote sum gradients.
    """
    batch_size = y_thermometer.shape[0]
    num_float_features = 2 * n
    
    # 1. Calculate Band Width
    # Range is [-3, 3], total span is 6.0
    band_width = 6.0 / num_bands
    
    # 2. Reshape and get base integer counts (Discrete Step)
    # Shape: (B, 16, 5)
    y_reshaped = y_thermometer.reshape((batch_size, num_float_features, num_bands))
    
    # Shape: (B, 16)
    num_ones = np.sum(y_reshaped, axis=2).astype(np.float64)
    
    # 3. Initialize Output Array
    y_continuous = np.zeros((batch_size, num_float_features), dtype=np.float64)
    
    # Extract baseline scores for the batch (Column 0)
    baseline_scores = scores_batch[:, 0]

    # Loop through each feature to calculate its specific offset
    for i in range(num_float_features):
        # Retrieve scores for moving this feature UP or DOWN
        scores_up = scores_batch[:, 2 * i + 1]
        scores_down = scores_batch[:, 2 * i + 2]
        
        # --- Vectorized Loop over Batch ---
        for b in range(batch_size):
            # A. Calculate the discrete base position
            # Matches your original logic: val = num_ones - 1
            # y = (val * band_width) - 3.0
            val_discrete = num_ones[b, i] - 1.0
            y_base = (val_discrete * band_width) - 3.0
            
            # B. Calculate Interpolation Offset using Quadratic Fit
            # We have 3 points: 
            #   f(x-1) = score_down
            #   f(x)   = baseline
            #   f(x+1) = score_up
            #
            # The vertex of the parabola is at x + offset, where:
            # offset = (S_down - S_up) / (2 * (S_down - 2*S_base + S_up))
            
            s_base = baseline_scores[b]
            s_up = scores_up[b]
            s_down = scores_down[b]
            
            numerator = s_down - s_up
            denominator = s_down + s_up - (2 * s_base)
            
            offset_fraction = 0.0
            
            # Avoid division by zero if the curve is perfectly linear or flat
            if abs(denominator) > 1e-6:
                offset_fraction = numerator / (2.0 * denominator)
            else:
                # Fallback to simple linear gradient logic if denominator is 0
                # If Up is better (lower score), move up slightly
                if s_up < s_base:
                    offset_fraction = 0.5
                elif s_down < s_base:
                    offset_fraction = -0.5
            
            # Clamp offset to [-0.5, 0.5] to ensure we stay within the 
            # logic of the current band (neighboring bands handle their own centers)
            if offset_fraction > 0.5:
                offset_fraction = 0.5
            elif offset_fraction < -0.5:
                offset_fraction = -0.5
                
            # C. Apply the offset to the base value
            # We add the fraction of the band width to the discrete center
            y_approx = y_base + (offset_fraction * band_width)
            
            # D. Store and global Clip to [-3, 3] bounds
            if y_approx > 3.0:
                y_approx = 3.0
            elif y_approx < -3.0:
                y_approx = -3.0
                
            y_continuous[b, i] = y_approx

    return y_continuous

def standardize_to_channel_dist(data, target_std=np.sqrt(0.5)):
    """
    Rescales data to have Mean=0 and Std=target_std.
    Matches the h_real and h_imag distribution:
    h ~ Normal(0, sqrt(0.5))
    """
    ...
    # 2. Normalize to Standard Normal (0, 1)
    z_score = (data - curr_mean) / curr_std
    
    # 3. Scale to specific channel distribution
    # New Mean = 0 (implied), New Std = target_std
    return z_score * target_std
    ...
    for epoch in range(epochs+1):
       ...
       #     LEARNING_RATE_AE = LEARNING_RATE_AE * 0.1
        ...
        if epoch % 3 == 0:
            ...
            # Apply learning rate smoothing to h
           # h_targets = h_est * (1 - LEARNING_RATE_AE_param) + h_targets * LEARNING_RATE_AE_param
            
            # Standardize targets
            if epoch < STANDARDIZE_EPOCHS_PARAM:
                h_targets[:, 0] = standardize_to_channel_dist(h_targets[:, 0])
                h_targets[:, 1] = standardize_to_channel_dist(h_targets[:, 1])
         ...
         #   y_optimized_reconstructed = (y_optimized_recon_np*LEARNING_RATE_AE_decoder + y_estimated*(1-LEARNING_RATE_AE_decoder))
           
         
            
            # We must use the per-sample scaled noise here
            #y_optimized_reconstructed = y_optimized_reconstructed - noise_scaled # <--- MODIFIED
            ...
            if channel_type in ['Rayleigh', 'Rician']:
              ...
              #  y_optimized_reconstructed_tensor = y_optimized_reconstructed_tensor - noise_scaled 
                y_optimized_reconstructed_tensor = revert_fading(y_optimized_reconstructed_tensor, h)
...
if __name__ == '__main__':
    ...
    evaluate_and_plot_bler_output = evaluate_and_plot_bler(
        ...
        save_path="bler_TM_vs_Benchmarks_R.png" 
    )
