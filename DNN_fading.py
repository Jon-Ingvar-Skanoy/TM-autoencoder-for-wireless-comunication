...
# --- (NEW) Final Training & Evaluation Parameters ---
# Use a longer epoch count for the final training run
EPOCHS_FINAL_TRAIN = 2000 

# Parameters for the evaluation phase
SNR_MIN_TEST = -20
SNR_MAX_TEST = 20
SNR_STEP_TEST = 1
N_TEST_BATCHES = 100       # Increase batches for a smoother curve
TEST_BATCH_SIZE = 4096    # Use a large batch size for eval
...
def train_autoencoder_standalone(encoder, decoder, epochs, snr_db_min, snr_db_max, channel_type, lr, batch_size):
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=300, min_lr=1e-6) # Longer patience for final run¨
    ...
    for epoch in range(epochs):
        ...
        # Only print log for the last epoch or specific intervals to reduce clutter in multi-run
        if (epoch + 1) % 200 == 0:
            _, s_hat_int = torch.max(s_hat_logits, 1)
            bler = (s_hat_int != s_int_batch).float().mean().item()
            print(f"   Epoch [{epoch+1}/{epochs}], Loss: {loss.item():.4f}, BLER: {bler:.4f}, SNR: {current_snr:.1f} dB")
...
# --- Evaluation Function ---
def evaluate_autoencoder(encoder, decoder, channel_type, snr_min_db, snr_max_db, snr_step, n_batches, batch_size):
    """
...
# --- Plotting Function ---
def plot_bler_curve(snr_values, bler_values, save_path="bler_plot_final_mean.png"):
    """
    Plots the BLER vs. SNR curve and saves it.
    """
    print(f"\n--- Phase 3: Plotting Mean Results ---")
    plt.figure(figsize=(10, 6))
    plt.semilogy(snr_values, bler_values, marker='o', linestyle='-', label=f'Mean BLER ({N_RUNS} runs)')
    
    plt.title(f'Mean BLER vs. SNR ({N_NORMAL},{K_NORMAL}) Autoencoder over {CHANNEL_TYPE}')
    plt.xlabel('SNR (dB)')
...
if __name__ == "__main__":
    ...
    N_RUNS = 3
    print(f"--- Starting {N_RUNS} Runs with Best Hyperparameters ---")
    
    all_bler_results = []
    final_snr_axis = None # Placeholder to store SNR values
    
    for run_i in range(N_RUNS):
        print(f"\n================ RUN {run_i + 1} / {N_RUNS} ================")
        
        # 1. Initialize networks (Fresh weights every run)
        ...
        # Capture the X-axis (SNR) once
        if final_snr_axis is None:
            final_snr_axis = snr_db
            
        all_bler_results.append(bler)
        
        # Calculate individual run performance
        ind_objective = np.trapezoid(bler, snr_db)
        print(f"Run {run_i + 1} Objective Value (Area): {ind_objective:.6f}")

    # --- 4. Process Multi-Run Results ---
    print("\n================ FINAL AGGREGATION ================")
    
    # Convert list of lists to numpy array for easy averaging
    all_bler_results = np.array(all_bler_results) # Shape: (3, num_snr_points)
    
    # Calculate Mean BLER column-wise
    mean_bler = np.mean(all_bler_results, axis=0)

    for bler, snr in zip(mean_bler, final_snr_axis):
        print(f"SNR: {snr:.1f} dB, Mean BLER: {bler:.6f}")

    
    # Calculate Mean Objective Value
    mean_objective_value = np.trapezoid(mean_bler, final_snr_axis)
    
    print(f"Mean Objective Value (Area under mean BLER curve): {mean_objective_value:.6f}")
    
    # Plot the mean results
    plot_bler_curve(final_snr_axis, mean_bler)
